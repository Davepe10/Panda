...
export const GAME_CATALOG: GameDef[] = [
  ...
  {id:'cards',title:'Cartas de Pandalandia',emoji:'🃏',description:'Imagina, decide, adivina, haz o arriésgate.',mode:'both',category:'creatividad',accent:'violet',points:15,minutes:'2–6 min'},
...
export const CHOICE_QUESTIONS: Record<string,{q:string;options:string[]}[]> = {
  ...
  'know-me': [
    ...
    {q:'¿Qué escogería para desayunar un domingo?',options:['Algo dulce','Algo salado','Solo café','Un desayuno grande']},
    {q:'¿Qué suelo valorar más al comprar algo?',options:['Precio','Calidad','Diseño','Que sea práctico']},
    {q:'¿Qué plan me atrae más un viernes por la noche?',options:['Cena fuera','Película en casa','Reunión con amigos','Salir sin plan']},
    {q:'¿Qué me molestaría más perder por una semana?',options:['Celular','Música','Streaming','Delivery']},
    {q:'¿Qué clima elegiría para unas vacaciones?',options:['Calor','Frío','Templado','Me da igual']},
    {q:'¿Qué elegiría primero en un buffet?',options:['Entrada','Plato fuerte','Postre','Probar de todo']},
    {q:'¿Qué tipo de película escogería más rápido?',options:['Comedia','Acción','Terror','Drama']},
    {q:'¿Qué me da más flojera hacer?',options:['Ordenar','Cocinar','Hacer trámites','Madrugar']},
    {q:'¿Qué mascota elegiría entre estas opciones?',options:['Perro','Gato','Conejo','Ninguna de esas']},
    {q:'¿Qué haría primero al llegar a un hotel?',options:['Revisar la habitación','Descansar','Salir a explorar','Buscar comida']},
    {q:'¿Qué aplicación abriría primero al tener un rato libre?',options:['Mensajes','Redes','Videos','Música']},
    {q:'¿Qué elegiría para moverme por una ciudad nueva?',options:['Caminar','Taxi/app','Transporte público','Bicicleta']},
    {q:'¿Qué me parece mejor para desconectarme?',options:['Dormir','Jugar','Ver algo','Salir']},
    {q:'¿Qué sabor elegiría más seguido?',options:['Dulce','Salado','Picante','Ácido']},
    {q:'¿Qué compraría antes si recibiera dinero extra?',options:['Tecnología','Ropa','Comida','Una experiencia']},
    {q:'¿Qué me atrae más de un restaurante?',options:['La comida','El ambiente','El precio','La novedad']},
    {q:'¿Qué haría con una tarde completamente libre?',options:['Dormir','Salir','Jugar/ver algo','Adelantar pendientes']},
    {q:'¿Qué me gustaría dominar instantáneamente?',options:['Un idioma','Un instrumento','Cocina','Un deporte']},
    {q:'¿Qué prefiero cuando tengo que decidir rápido?',options:['Seguir intuición','Comparar opciones','Preguntar','Dejarlo al azar']},
    {q:'¿Qué elegiría para una escapada corta?',options:['Playa','Montaña','Ciudad','Campo']},
    {q:'¿Qué me resulta más entretenido?',options:['Competir','Resolver acertijos','Crear algo','Explorar']},
    {q:'¿Qué bebida escogería primero?',options:['Agua','Gaseosa','Café','Jugo']},
    {q:'¿Qué me importa más en un celular?',options:['Cámara','Batería','Rendimiento','Diseño']},
    {q:'¿Qué tipo de regalo sorpresa me gustaría más?',options:['Comida favorita','Entrada a un evento','Algo útil','Una salida']},
    {q:'¿Qué elegiría si solo pudiera llevar una cosa a un viaje largo?',options:['Audífonos','Cargador','Almohada','Snacks']},
    {q:'¿Qué actividad me parece más divertida?',options:['Karaoke','Bowling','Cine','Escape room']},
    {q:'¿Qué prefiero para organizar un plan?',options:['Todo definido','Solo lo básico','Improvisar','Que otro organice']},
    {q:'¿Qué elegiría para pasar una hora sin internet?',options:['Dormir','Conversar','Caminar','Leer/jugar offline']},
    {q:'¿Qué me haría más ilusión aprender?',options:['Fotografía','Baile','Cocina','Otro idioma']},
    {q:'¿Qué elegiría como premio después de una semana pesada?',options:['Comida especial','Dormir mucho','Salir','Comprar algo']},
  ]
}
...
export const TRIVIA_CURATED_COUNT = 36
...
// -----------------------------------------------------------------------------
// Pandalandia V10 · bancos extensos y deterministas
// Cada juego basado en contenido tiene >= 3,200 entradas disponibles en cliente.
// No se inventan recuerdos de la pareja: todo es genérico, hipotético o factual.
// -----------------------------------------------------------------------------
const BANK_TARGET = 3200
function fillBank<T>(target:T[], make:(i:number)=>T, key:(x:T)=>string){
  const seen=new Set(target.map(key));
  let i=0,guard=0;
  while(target.length<BANK_TARGET&&guard<BANK_TARGET*20){const item=make(i++);const k=key(item);if(!seen.has(k)){seen.add(k);target.push(item)}guard++}
}

const plans=['ver una película','salir a caminar','probar un restaurante','cocinar algo nuevo','jugar videojuegos','visitar un museo','hacer una escapada','ir a un concierto','quedarme en casa','hacer deporte','leer','tomar fotos','ir a la playa','ir a la montaña','hacer una ruta de comida','ir de compras','visitar un parque','hacer karaoke','ir al cine','hacer un picnic','ir a una feria','jugar un juego de mesa','hacer una clase nueva','ir a una cafetería','ver una serie','escuchar música','hacer una sesión de fotos','explorar un barrio','ir a bailar','descansar sin planes','hacer una manualidad','armar un rompecabezas','ir a un mirador','visitar una librería','hacer una parrilla','ir a bowling','jugar billar','hacer una caminata larga','probar un postre','ver el atardecer']
const moods=['después de una semana pesada','en un domingo libre','si mañana no hubiera obligaciones','para celebrar algo pequeño','si lloviera todo el día','si tuvieras mucha energía','si quisieras desconectarte','si solo tuvieras dos horas','si pudieras improvisar','para cambiar de rutina','en vacaciones','en una tarde tranquila','cuando necesitas reírte','si el presupuesto fuera libre','con poco presupuesto','si hiciera mucho frío','si hiciera calor','si estuvieras de muy buen humor','cuando quieres algo simple','si quisieras probar algo distinto']
const preferences=['aventura','comodidad','sorpresa','organización','tranquilidad','actividad','creatividad','competencia','comida','naturaleza','tecnología','música','cine','viajes','deporte','descanso','socializar','aprender','explorar','improvisar','fotografía','juegos','cultura','humor','romance','misterio','velocidad','estrategia','arte','aire libre','ciudad','campo','mar','montaña','dulce','salado','día','noche','frío','calor']

fillBank(CHOICE_QUESTIONS['would-you-rather'],i=>{const a=plans[i%plans.length],b=plans[(i*7+11)%plans.length],m=moods[(i*13)%moods.length];return{q:`${m}, ¿qué preferirías? #${i+1}`,options:[a,b]}},x=>x.q+'|'+x.options.join('|'))
fillBank(CHOICE_QUESTIONS['likely'],i=>{const p=plans[i%plans.length],m=moods[(i*9+3)%moods.length],trait=preferences[(i*17)%preferences.length];return{q:`¿Quién es más probable que elija ${p} ${m} si la clave fuera ${trait}? #${i+1}`,options:['Yo','Mi persona']}},x=>x.q)
fillBank(CHOICE_QUESTIONS['either-or'],i=>{const a=preferences[i%preferences.length],b=preferences[(i*11+7)%preferences.length],m=moods[(i*5)%moods.length];return{q:`${m}: elige sin pensarlo demasiado #${i+1}`,options:[a,b]}},x=>x.q+'|'+x.options.join('|'))
fillBank(CHOICE_QUESTIONS['know-me'],i=>{const m=moods[i%moods.length],a=plans[(i*3)%plans.length],b=plans[(i*5+1)%plans.length],c=plans[(i*7+2)%plans.length],d=plans[(i*11+3)%plans.length];return{q:`¿Qué crees que escogería ${m}? #${i+1}`,options:[a,b,c,d]}},x=>x.q+'|'+x.options.join('|'))

fillBank(MIND_PROMPTS,i=>{const p=preferences[i%preferences.length],m=moods[(i*7)%moods.length],a=plans[(i*13)%plans.length];return`Piensa rápido: algo relacionado con ${p}, ${m}, y ${a} #${i+1}`},x=>x)

const drawNouns=['panda','gato','perro','capibara','pingüino','cohete','castillo','robot','jirafa','volcán','bicicleta','fantasma','tiburón','helicóptero','sandía','dragón','cactus','corona','hamburguesa','planeta','guitarra','pulpo','dinosaurio','ballena','pirata','helado','mariposa','avión','barco','tren','casa','árbol','luna','sol','estrella','nube','zapato','sombrero','taza','pizza','torta','flor','pez','conejo','león','mono','serpiente','rana','caballo','oveja','elefante','ratón','búho','abeja','caracol','cangrejo','tortuga','cofre','espada','escudo','torre','puente','isla','montaña','río','cascada','paraguas','reloj','teléfono','computadora','cámara','mochila','globo','regalo','vela','libro','pelota','patineta']
const drawMods=['con sombrero','bailando','durmiendo','comiendo pizza','en la playa','en el espacio','bajo la lluvia','con lentes','en bicicleta','tocando guitarra','en una fiesta','leyendo','corriendo','volando','en un barco','con una corona','tomando café','jugando fútbol','en una montaña','dentro de una caja','con una flor','en la Luna','junto a un arcoíris','cocinando','cantando','haciendo ejercicio','en una piscina','con una mochila','en una nube','en un castillo','en un parque','viendo una película','con un helado','en un tren','en un avión','en una isla','con un regalo','bajo las estrellas','frente a un volcán','en una cafetería']
fillBank(DRAW_WORDS,i=>`${drawNouns[i%drawNouns.length]} ${drawMods[Math.floor(i/drawNouns.length)%drawMods.length]} #${Math.floor(i/(drawNouns.length*drawMods.length))+1}`,x=>x)

const tabooWords=['PLAYA','PIZZA','AVIÓN','PERRO','CAFÉ','CELULAR','FÚTBOL','LUNA','HOSPITAL','CHOCOLATE','PANDA','MÚSICA','PELÍCULA','VIAJE','COCINA','FIESTA','LIBRO','MONTAÑA','HELADO','COMPUTADORA','TREN','BARCO','GATO','FOTO','RESTAURANTE','PARQUE','BICICLETA','ROBOT','DRAGÓN','CASA','AMIGO','ESCUELA','TRABAJO','DINERO','TIEMPO','LLUVIA','SOL','NOCHE','CIUDAD','CAMPO']
const tabooForbidden=['decir','mostrar','nombre','objeto','lugar','acción','color','sonido','persona','comida','viaje','pantalla','animal','juego','rápido','grande','pequeño','famoso','usar','hacer']
fillBank(TABOO_CARDS,i=>{const word=`${tabooWords[i%tabooWords.length]} ${Math.floor(i/tabooWords.length)+1}`;const a=tabooForbidden[(i*3)%tabooForbidden.length],b=tabooForbidden[(i*5+1)%tabooForbidden.length],c=tabooForbidden[(i*7+2)%tabooForbidden.length];return{word,forbidden:[a,b,c]}},x=>x.word)

function triviaItem(i:number){
  const mode=i%4,j=Math.floor(i/4),shift=(j*3+mode)%4;
  if(mode===0){const a=11+(j%97),b=13+Math.floor(j/97),ans=a+b,opts=[ans,ans+1,ans-1,ans+10];return{q:`¿Cuánto es ${a} + ${b}?`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  if(mode===1){const a=250+j,b=17+((j*7)%53),ans=a-b,opts=[ans,ans+2,ans-2,ans+5];return{q:`¿Cuánto es ${a} − ${b}?`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  if(mode===2){const a=2+(j%31),b=2+(Math.floor(j/31)%31),ans=a*b,opts=[ans,ans+a,Math.max(0,ans-b),ans+1];return{q:`¿Cuánto es ${a} × ${b}?`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  const start=3+(j%41),step=2+(Math.floor(j/41)%20),ans=start+step*4,opts=[ans,ans+step,ans-step,ans+1];return{q:`Completa la secuencia: ${start}, ${start+step}, ${start+step*2}, ${start+step*3}, …`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Lógica'}
}
fillBank(TRIVIA,triviaItem,x=>x.q)

const cardTypes=['IMAGINA','DECIDE','HAZ','ADIVINA','RIESGO'] as const
fillBank(CARD_DECK,i=>{const type=cardTypes[i%cardTypes.length],p=plans[(i*7)%plans.length],m=moods[(i*11)%moods.length],pref=preferences[(i*13)%preferences.length];const text=type==='IMAGINA'?`Imaginen una versión absurda de ${p} ${m}. Debe incluir algo relacionado con ${pref}.`:type==='DECIDE'?`Decidan juntos entre ${p} y ${plans[(i*5+9)%plans.length]} ${m}.`:type==='HAZ'?`Haz durante 20 segundos una mini actuación relacionada con ${p} y ${pref}.`:type==='ADIVINA'?`Piensa en algo relacionado con ${pref}. Da tres pistas sin decir la palabra.`:`Deja que ${'la otra persona'} elija entre ${p} y ${plans[(i*3+4)%plans.length]}.`;return{type,text:`${text} #${i+1}`}},x=>x.type+'|'+x.text)

const truthKinds=['Verdad','Reto','Sorpresa'] as const
fillBank(TRUTH_DARES,i=>{const kind=truthKinds[i%truthKinds.length],p=plans[(i*7)%plans.length],m=moods[(i*11)%moods.length],pref=preferences[(i*13)%preferences.length];const text=kind==='Verdad'?`¿Qué opinas de ${p} ${m}, especialmente si importa ${pref}?`:kind==='Reto'?`Haz un reto de 20 segundos inspirado en ${p} y ${pref}.`:`Elijan juntos algo relacionado con ${p} y ${pref} para hacer ${m}.`;return{kind,text:`${text} #${i+1}`}},x=>x.kind+'|'+x.text)

export const CONTENT_BANK_SIZES = {
  wouldYouRather: CHOICE_QUESTIONS['would-you-rather'].length,
  likely: CHOICE_QUESTIONS['likely'].length,
  eitherOr: CHOICE_QUESTIONS['either-or'].length,
  knowMe: CHOICE_QUESTIONS['know-me'].length,
  mindMeld: MIND_PROMPTS.length,
  draw: DRAW_WORDS.length,
  taboo: TABOO_CARDS.length,
  trivia: TRIVIA.length,
  cards: CARD_DECK.length,
  truthDare: TRUTH_DARES.length,
}
