...
function triviaItem(i:number){
  const mode=i%4;
  if(mode===0){const a=17+(i%83),b=9+((i*7)%91),ans=a+b,opts=[ans,ans+1,ans-1,ans+10];const shift=i%4;return{q:`¿Cuánto es ${a} + ${b}? #${i+1}`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  if(mode===1){const a=40+(i%160),b=2+((i*5)%38),ans=a-b,opts=[ans,ans+2,ans-2,ans+5];const shift=i%4;return{q:`¿Cuánto es ${a} − ${b}? #${i+1}`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  if(mode===2){const a=2+(i%11),b=2+((i*3)%11),ans=a*b,opts=[ans,ans+a,ans-b,ans+1];const shift=i%4;return{q:`¿Cuánto es ${a} × ${b}? #${i+1}`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Cálculo'}}
  const start=3+(i%30),step=2+((i*7)%9),ans=start+step*4,opts=[ans,ans+step,ans-step,ans+1];const shift=i%4;return{q:`Completa la secuencia: ${start}, ${start+step}, ${start+step*2}, ${start+step*3}, … #${i+1}`,o:Array.from({length:4},(_,k)=>String(opts[(k-shift+4)%4])),a:shift,cat:'Lógica'}
}
