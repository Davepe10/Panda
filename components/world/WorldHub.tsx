...
export default function WorldHub({coupleId,userId,partnerName='Tu compañero/a'}:{coupleId:string;userId:string;partnerName?:string}){
 ...
 const [careFx,setCareFx]=useState<'feed'|'water'|'clean'|''>('')
 ...
 async function act(kind:'feed'|'water'|'play'|'pet'|'rest'|'clean'|'care'){
  const anim:Record<string,string>={feed:'eat',water:'drink',play:'play',pet:'wag',rest:'sleep',clean:'clean',care:'sit'}
  if(kind==='feed'||kind==='water'||kind==='clean'){setCareFx(kind);window.setTimeout(()=>setCareFx(''),kind==='clean'?4300:3600)}
  animatePet(anim[kind]||'idle',kind==='rest'?6200:kind==='clean'?4300:3800)
  await rpc('pet_interact',{p_action:kind})
 }
 ...
 return <div className="world-shell world-shell-v28 world-shell-v29 world-shell-v30" data-world-tab={tab}>
  ...
  {tab==='home'&&<>
   <div className="pet-home-premium-v63">
    <section className={`pet-stage-premium-v63 theme-${world.room_theme||'warm'} ${specialFx?`pet-special-${specialFx}`:''} ${careFx?`pet-care-fx-${careFx}`:''}`}>
      <div className="pet-room-3d-layer"><PetRoom3D type={world.pet_type} mood={visualMood} items={equipped} growthStage={world.growth_stage} behavior={petBehavior} behaviorNonce={petBehaviorNonce} onPetInteract={handlePetTouch} onFeed={()=>{if(!busy)void act('feed')}} onWater={()=>{if(!busy)void act('water')}}/></div>
      {careFx&&<div className={`pet-care-feedback ${careFx}`} aria-live="polite"><span>{careFx==='feed'?'🥣':careFx==='water'?'💧':'🫧'}</span><b>{careFx==='feed'?'Hora de comer':careFx==='water'?'Tomando agua':'Baño en progreso'}</b><small>{careFx==='clean'?'Espuma, cuidado y una buena sacudida al terminar.':`${world.pet_name||'Tu mascota'} se acerca por sí sola.`}</small></div>}
    ...
    <section className="pet-needs-v63">
      ...
      <div className="pet-care-title"><b>Cuidados</b><span>Siempre disponibles · se destaca lo que más necesita</span></div>
      <div className="pet-care-dock-v63 pet-care-dock-v67">
       <button className={world.hunger<45?'recommended':''} disabled={busy} onClick={()=>act('feed')}><span>🥣</span><b>Dar comida</b><small>{world.hunger<45?'Tiene hambre':'Alimentarla'}</small></button>
       <button className={world.thirst<45?'recommended':''} disabled={busy} onClick={()=>act('water')}><span>💧</span><b>Dar agua</b><small>{world.thirst<45?'Tiene sed':'Hidratarla'}</small></button>
       <button className={world.energy<35?'recommended':''} disabled={busy} onClick={()=>act('rest')}><span>🌙</span><b>Descansar</b><small>{world.energy<35?'Necesita energía':'Tomar una siesta'}</small></button>
       <button className={world.hygiene<45?'recommended':''} disabled={busy} onClick={()=>act('clean')}><span>🫧</span><b>Bañar</b><small>{world.hygiene<45?'Necesita un baño':'Mantenerla limpia'}</small></button>
       <button className={world.fun<45?'recommended':''} disabled={busy} onClick={()=>act('play')}><span>🎾</span><b>Jugar</b><small>{world.fun<45?'Quiere divertirse':'Divertirse'}</small></button>
       <button className={world.affection<45?'recommended':''} disabled={busy} onClick={()=>act('pet')}><span>🤍</span><b>Acariciar</b><small>{world.affection<45?'Busca cariño':'Darle afecto'}</small></button>
      </div>
      {world.sickness_status!=='healthy'&&<button className="pet-health-action" disabled={busy} onClick={()=>act('care')}>🧰 <b>Cuidar a {world.pet_name}</b><span>Necesita atención especial</span></button>}
    </section>
   </div>
