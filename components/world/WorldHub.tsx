  ...
  {tab==='home'&&<>
   <div className="pet-home-premium-v63">
    <section className={`pet-stage-premium-v63 theme-${world.room_theme||'warm'} ${specialFx?`pet-special-${specialFx}`:''}`}>
      <div className="pet-room-3d-layer"><PetRoom3D type={world.pet_type} mood={visualMood} items={equipped} growthStage={world.growth_stage} behavior={petBehavior} behaviorNonce={petBehaviorNonce}/></div>
      <button type="button" className="pet-touch-zone" aria-label={`Acariciar a ${world.pet_name}`} onClick={()=>{animatePet('pet',2600,false);if(typeof navigator!=='undefined'&&'vibrate' in navigator)navigator.vibrate?.(18)}}/>
   ...
   <section className="pet-autonomy-v63"><div><span className="pet-autonomy-icon">✦</span><div><h2>Se comporta por sí sola</h2><p>No necesitas pulsar “caminar” o “ladrar”. Mia decide cuándo recorrer la habitación, olfatear, sentarse, mover la cola, jugar o descansar según cómo se siente.</p></div></div><span className="pet-autonomy-status"><i/> {behaviorLabel(petBehavior,world.pet_sex)}</span></section>
