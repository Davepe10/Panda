...
export default function WorldHub({coupleId,userId,partnerName='Tu compañero/a'}:{coupleId:string;userId:string;partnerName?:string}){
 ...
 function animatePet(kind:string,duration=3200){setPetBehavior(kind);setPetBehaviorNonce(n=>n+1);window.setTimeout(()=>setPetBehavior('idle'),duration)}
 ...
 async function act(kind:'feed'|'water'|'play'|'pet'|'rest'|'clean'|'care'){const anim:Record<string,string>={feed:'eat',water:'drink',play:'play',pet:'wag',rest:'sleep',clean:'scratch',care:'sit'};animatePet(anim[kind]||'idle',kind==='rest'?6200:3800);await rpc('pet_interact',{p_action:kind})}
