...
function PremiumCocker({mood='happy',behavior='idle',behaviorNonce=0,onInteract}:{mood?:Mood;behavior?:PetBehavior;behaviorNonce?:number;onInteract?:()=>void}){
 ...
 useEffect(()=>{
  if(['walk','run','play','sniff'].includes(String(behavior)))chooseWaypoint(behavior==='run'||behavior==='play')
  nextWaypointAt.current=0
 },[behavior,behaviorNonce])
 useFrame(({clock},delta)=>{
  const t=clock.elapsedTime,g=root.current;if(!g)return
  const locomotion=behavior==='walk'||behavior==='run'||behavior==='play'||behavior==='sniff',run=behavior==='run'||behavior==='play',sit=behavior==='sit'||behavior==='care',sleep=behavior==='sleep'
  ...
  if(locomotion){
   ...
   if((dist<.20||t>nextWaypointAt.current)&&t>nextWaypointAt.current){chooseWaypoint(run);nextWaypointAt.current=t+(run?1.7:behavior==='sniff'?3.0:2.5)}
   const speed=run?3.8:behavior==='sniff'?1.45:2.35
...
export function PetModel({type,mood='happy',interactive=true,growthStage='young',items=[],behavior='idle',behaviorNonce=0,onInteract}:PetModelProps&{growthStage?:GrowthStage}){
 ...
 return <group ref={root}><HighFidelityPet type={type} mood={mood} behavior={behavior} behaviorNonce={behaviorNonce} onInteract={interactive?onInteract:undefined}/><Wearables items={items}/><PetStatusEffects mood={mood}/></group>
}
...
function RoomScene({type,mood,items,growthStage,behavior,behaviorNonce,onPetInteract}:{type:PetType;mood:Mood;items:string[];growthStage:GrowthStage;behavior:PetBehavior;behaviorNonce:number;onPetInteract?:()=>void}){
