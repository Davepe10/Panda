...
if(typeof window!=='undefined'){useGLTF.preload('/models/pets/cocker.glb');useGLTF.preload('/models/pets/cocker-real-stand.glb');useGLTF.preload('/models/pets/cocker-real-sit.glb');if((navigator.hardwareConcurrency||6)>4&&window.innerWidth>=360){useGLTF.preload('/models/pets/cocker-real-stand-hd.glb');useGLTF.preload('/models/pets/cocker-real-sit-hd.glb')}}
...
function RiggedPetAsset({type,mood='happy',behavior='idle'}:{type:PetType;mood?:Mood;behavior?:PetBehavior}){
 ...
 useFrame((state,delta)=>{
  ...
  if(cockerParts){
   ...
   }else if(scratch){const fr=part('Leg_FR');if(fr)fr.rotation.x+=.35+Math.sin(t*10)*.45}
  }
  void delta
 })
...
function RealSculptCocker({mood='happy',behavior='idle'}:{mood?:Mood;behavior?:PetBehavior}){
 const [highDetail,setHighDetail]=useState(false)
 useEffect(()=>{
  const pick=()=>setHighDetail((navigator.hardwareConcurrency||6)>4 && window.innerWidth>=360)
  pick();window.addEventListener('resize',pick,{passive:true});return()=>window.removeEventListener('resize',pick)
 },[])
 const standUrl=highDetail?'/models/pets/cocker-real-stand-hd.glb':'/models/pets/cocker-real-stand.glb'
 const sitUrl=highDetail?'/models/pets/cocker-real-sit-hd.glb':'/models/pets/cocker-real-sit.glb'
 const standing=useGLTF(standUrl)
 const sitting=useGLTF(sitUrl)
 const stand=useMemo(()=>standing.scene.clone(true),[standing.scene])
 const sit=useMemo(()=>sitting.scene.clone(true),[sitting.scene])
 const root=useRef<THREE.Group>(null)
 const visual=useRef<THREE.Group>(null)
 const useSit=['sit','sleep','care'].includes(String(behavior))
 const model=useSit?sit:stand
 const normalization=useMemo(()=>{
  model.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(model),size=new THREE.Vector3(),center=new THREE.Vector3();box.getSize(size);box.getCenter(center)
  const scale=size.y>0?1.70/size.y:1
  return {scale,offset:new THREE.Vector3(-center.x*scale,-box.min.y*scale-.94,-center.z*scale)}
 },[model])
 const material=useMemo(()=>{
  // V32: the STL has no UVs, so the coat is shaded in object space. Layered
  // directional noise avoids the single-color "clay/mancha" look without
  // requiring a fake stretched 2D texture. The geometry remains the user's HD sculpt.
  const m=new THREE.MeshPhysicalMaterial({
   color:'#b66b3b',roughness:.91,metalness:0,sheen:1,sheenRoughness:.92,
   sheenColor:new THREE.Color('#f0bd91'),clearcoat:.015,clearcoatRoughness:.95
  })
  m.onBeforeCompile=(shader)=>{
   shader.vertexShader=shader.vertexShader
    .replace('#include <common>','#include <common>\nvarying vec3 vPetLocal;\nvarying vec3 vPetNormal;')
    .replace('#include <begin_vertex>','#include <begin_vertex>\nvPetLocal=position;\nvPetNormal=normal;')
   shader.fragmentShader=shader.fragmentShader
    .replace('#include <common>','#include <common>\nvarying vec3 vPetLocal;\nvarying vec3 vPetNormal;')
    .replace('vec4 diffuseColor = vec4( diffuse, opacity );',`
float strandA = sin(vPetLocal.y*118.0 + vPetLocal.x*37.0 + sin(vPetLocal.z*21.0)*2.4)*.5+.5;
float strandB = sin(vPetLocal.y*67.0 - vPetLocal.z*91.0 + sin(vPetLocal.x*33.0)*1.8)*.5+.5;
float strandC = sin((vPetLocal.x+vPetLocal.z)*143.0 + vPetLocal.y*18.0)*.5+.5;
float macro = sin(vPetLocal.x*8.0 + sin(vPetLocal.z*6.0))*0.5+0.5;
float coat = clamp(strandA*.42 + strandB*.32 + strandC*.14 + macro*.12,0.0,1.0);
vec3 caramel = vec3(.66,.30,.12);
vec3 copper = vec3(.88,.49,.23);
vec3 highlight = vec3(.98,.72,.47);
vec3 furTint = mix(caramel,copper,smoothstep(.18,.82,coat));
furTint = mix(furTint,highlight,pow(max(dot(normalize(vPetNormal),normalize(vec3(.15,.85,.32))),0.0),4.0)*.16);
float feather = smoothstep(.30,.95,abs(vPetNormal.x)) * strandB;
furTint *= .90 + feather*.13;
vec4 diffuseColor = vec4(diffuse * furTint, opacity);`)
  }
  m.customProgramCacheKey=()=> 'pandalandia-cocker-fur-v32-realcoat'
  return m
 },[])
 useEffect(()=>{model.traverse((o:any)=>{if(o?.isMesh){o.castShadow=true;o.receiveShadow=true;if(o.geometry&&!o.geometry.getAttribute('normal'))o.geometry.computeVertexNormals();o.material=material}});return()=>material.dispose()},[model,material])
 useFrame(({clock},delta)=>{
  const g=visual.current;if(!g)return;const t=clock.elapsedTime
  const locomotion=behavior==='walk'||behavior==='run', eating=behavior==='eat'||behavior==='drink', bark=behavior==='bark', sniff=behavior==='sniff', pant=behavior==='pant', lick=behavior==='lick', scratch=behavior==='scratch', stretch=behavior==='stretch', wag=behavior==='wag'||behavior==='pet'||behavior==='play'
  const x=locomotion?Math.sin(t*(behavior==='run'?1.35:.75))*.30:eating?(behavior==='eat'?.48:.76):0
  g.position.x=THREE.MathUtils.damp(g.position.x,x,5.5,delta)
  g.position.y=THREE.MathUtils.damp(g.position.y,eating?-.10:behavior==='sleep'?-.18:stretch?-.06:0,6,delta)
  g.rotation.x=THREE.MathUtils.damp(g.rotation.x,eating?.30:sniff?.16:stretch?.10:behavior==='sleep'?.16:0,6,delta)
  g.rotation.y=THREE.MathUtils.damp(g.rotation.y,locomotion?Math.sin(t*.8)*.07:eating?-.18:sniff?Math.sin(t*2.4)*.10:0,6,delta)
  g.rotation.z=THREE.MathUtils.damp(g.rotation.z,scratch?Math.sin(t*7)*.035:wag?Math.sin(t*7.5)*.018:0,7,delta)
  const bounce=locomotion?Math.abs(Math.sin(t*(behavior==='run'?10:6)))*(behavior==='run'?.045:.018):bark?Math.abs(Math.sin(t*8))*.018:pant?Math.sin(t*4)*.006:lick?Math.sin(t*5)*.008:0
  g.position.y+=bounce
 })
 return <group ref={root} position={normalization.offset} scale={normalization.scale}><group ref={visual}><primitive object={model}/></group></group>
}
...
function HighFidelityPet({type,mood,behavior='idle'}:{type:PetType;mood:Mood;behavior?:PetBehavior}){
 if(type==='cocker')return <PetAssetBoundary fallback={<ProceduralPet type={type} mood={mood}/>}><Suspense fallback={<ProceduralPet type={type} mood={mood}/>}><RealSculptCocker mood={mood} behavior={behavior}/></Suspense></PetAssetBoundary>
 const asset=RIGGED_PET_ASSETS[type]
