...
type PetModelProps={type:PetType;mood?:Mood;interactive?:boolean;items?:string[];behavior?:PetBehavior;behaviorNonce?:number;onInteract?:()=>void}
...
function PremiumCocker({mood='happy',behavior='idle',behaviorNonce=0,onInteract}:{mood?:Mood;behavior?:PetBehavior;behaviorNonce?:number;onInteract?:()=>void}){
 ...
 const sleepy=mood==='sleepy'||mood==='sick'
 const chooseWaypoint=(wide=false)=>{
  const x=(Math.random()*2-1)*(wide?1.72:1.45)
  const z=-.44+Math.random()*1.18
  target.current.set(x,0,z)
 }
 useEffect(()=>{
  if(behavior==='eat')target.current.set(.72,0,.58)
  else if(behavior==='drink')target.current.set(1.32,0,.58)
  else if(['walk','run','play','sniff'].includes(String(behavior)))chooseWaypoint(behavior==='run'||behavior==='play')
  nextWaypointAt.current=0
 },[behavior,behaviorNonce])
 useFrame(({clock},delta)=>{
  const t=clock.elapsedTime,g=root.current;if(!g)return
  const approach=behavior==='eat'||behavior==='drink',locomotion=behavior==='walk'||behavior==='run'||behavior==='play'||behavior==='sniff'||approach,run=behavior==='run'||behavior==='play',sit=behavior==='sit'||behavior==='care',sleep=behavior==='sleep'
  const freq=run?9.6:behavior==='sniff'?4.2:6.4,amp=run?.66:behavior==='sniff'?.28:.44
  legs.forEach((r,i)=>{if(!r.current)return;const phase=i%2===0?0:Math.PI;r.current.rotation.x=THREE.MathUtils.damp(r.current.rotation.x,locomotion?Math.sin(t*freq+phase)*amp:sit?.18:sleep?.34:0,9,delta)})
  if(tail.current)tail.current.rotation.z=THREE.MathUtils.damp(tail.current.rotation.z,['wag','play','pet','bark','run'].includes(String(behavior))?Math.sin(t*(run?13:10))*.54:Math.sin(t*2.2)*.18,8,delta)
  if(head.current){head.current.rotation.x=THREE.MathUtils.damp(head.current.rotation.x,behavior==='sniff'?.30:behavior==='eat'||behavior==='drink'?.21:sleep?.18:behavior==='bark'?-.10:0,6,delta);head.current.rotation.y=THREE.MathUtils.damp(head.current.rotation.y,behavior==='idle'?Math.sin(t*.55)*.12:behavior==='pet'?Math.sin(t*1.8)*.08:0,5,delta)}
  if(earL.current)earL.current.rotation.z=.16+Math.sin(t*(locomotion?freq:1.8))*(locomotion?.13:.04)
  if(earR.current)earR.current.rotation.z=-.16-Math.sin(t*(locomotion?freq:1.8))*(locomotion?.13:.04)
  if(locomotion){
   const dx=target.current.x-g.position.x,dz=target.current.z-g.position.z,dist=Math.hypot(dx,dz)
   if(!approach&&(dist<.20||t>nextWaypointAt.current)&&t>nextWaypointAt.current){chooseWaypoint(run);nextWaypointAt.current=t+(run?1.7:behavior==='sniff'?3.0:2.5)}
   const speed=approach?4.2:run?3.8:behavior==='sniff'?1.45:2.35
   g.position.x=THREE.MathUtils.damp(g.position.x,target.current.x,speed,delta)
   g.position.z=THREE.MathUtils.damp(g.position.z,target.current.z,speed,delta)
   const yaw=Math.atan2(target.current.x-g.position.x,target.current.z-g.position.z)
   g.rotation.y=THREE.MathUtils.damp(g.rotation.y,yaw,6,delta)
  }
  const baseY=sleep?-.20:sit?-.08:locomotion?Math.abs(Math.sin(t*freq))*0.05:Math.sin(t*2.1)*.008
  g.position.y=THREE.MathUtils.damp(g.position.y,baseY,7,delta)
  g.rotation.z=THREE.MathUtils.damp(g.rotation.z,behavior==='scratch'?Math.sin(t*8)*.08:behavior==='pet'?Math.sin(t*2.4)*.025:0,6,delta)
 })
 const Leg=({r,x,z,dark=false}:{r:RefObject<THREE.Group|null>;x:number;z:number;dark?:boolean})=><group ref={r} position={[x,-.55,z]}>
   <mesh position={[0,-.18,0]} scale={[.23,.58,.23]} castShadow><capsuleGeometry args={[.18,.48,8,16]}/><meshPhysicalMaterial {...fur(dark?'#78422f':'#bd744e')}/></mesh>
   <Paw position={[0,-.55,.08]} color={dark?'#75412f':'#bd744e'}/>
 </group>
 return <group ref={root} scale={.92} onPointerDown={(e)=>{e.stopPropagation();onInteract?.()}}>
  <mesh position={[0,-.12,-.06]} scale={[.86,.70,1.16]} castShadow><sphereGeometry args={[.70,48,34]}/><meshPhysicalMaterial {...fur('#b96f49')}/></mesh>
  <mesh position={[0,-.03,.55]} scale={[.56,.64,.46]} castShadow><sphereGeometry args={[.54,42,30]}/><meshPhysicalMaterial {...fur('#ead0b5')}/></mesh>
  <group ref={head} position={[0,.50,.42]}>
   <mesh scale={[.74,.77,.72]} castShadow><sphereGeometry args={[.62,48,34]}/><meshPhysicalMaterial {...fur('#c98258')}/></mesh>
   <group ref={earL} position={[-.49,.02,-.03]}><mesh scale={[.44,1.08,.28]} castShadow><sphereGeometry args={[.40,38,26]}/><meshPhysicalMaterial {...fur('#70402f',.96)}/></mesh></group>
   <group ref={earR} position={[.49,.02,-.03]}><mesh scale={[.44,1.08,.28]} castShadow><sphereGeometry args={[.40,38,26]}/><meshPhysicalMaterial {...fur('#70402f',.96)}/></mesh></group>
...
export function PetModel({type,mood='happy',interactive=true,growthStage='young',items=[],behavior='idle',behaviorNonce=0,onInteract}:PetModelProps&{growthStage?:GrowthStage}){
 const root=useRef<THREE.Group>(null)
 ...
 useFrame(({clock},delta)=>{
  ...
  const targetY=type==='cocker'?(growthStage==='baby'?-.04:growthStage==='adult'?.24:.15):(growthStage==='baby'?-.10:growthStage==='adult'?.10:.04)
...
function Furniture({items,onFeed,onWater}:{items:string[];onFeed?:()=>void;onWater?:()=>void}){
 ...
 return <>
  ...
  <group position={[1.02,-.89,.88]}>
   <group onPointerDown={(e)=>{e.stopPropagation();onFeed?.()}}>
   ...
   <group position={[.62,0,0]} onPointerDown={(e)=>{e.stopPropagation();onWater?.()}}>
   <mesh castShadow><cylinderGeometry args={[.27,.22,.11,28]}/>{metal('#c7dbe6',.18)}</mesh>
   <mesh position={[0,.07,0]}><cylinderGeometry args={[.205,.205,.018,28]}/><meshPhysicalMaterial color="#82cce5" roughness={.06} transmission={.18} clearcoat={1}/></mesh>
   </group>
  </group>
...
function RoomArchitecture({night}:{night:boolean}){
 const wall=night?'#7f7888':'#eee2da',floor=night?'#8e756c':'#cfae8f',sky=night?'#6f83a6':'#a8dced'
 return <>
...
function RoomScene({type,mood,items,growthStage,behavior,behaviorNonce,onPetInteract,onFeed,onWater}:{type:PetType;mood:Mood;items:string[];growthStage:GrowthStage;behavior:PetBehavior;behaviorNonce:number;onPetInteract?:()=>void;onFeed?:()=>void;onWater?:()=>void}){
 const hour=new Date().getHours(),night=hour<7||hour>=19
 return <>
  <PBREnvironment intensity={night?.58:.76}/>
  <color attach="background" args={[night?'#677489':'#eaf1f3']}/><fog attach="fog" args={[night?'#677489':'#eaf1f3',7.5,13]}/>
  ...
  <RoomArchitecture night={night}/><Furniture items={items} onFeed={onFeed} onWater={onWater}/>
  <PetModel type={type} mood={mood} growthStage={growthStage} items={items} behavior={behavior} behaviorNonce={behaviorNonce} onInteract={onPetInteract}/>
